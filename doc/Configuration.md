# Configuration Guide <br/> Feedbacks Microservice

Feedbacks microservice configuration structure follows the 
...
Example to retrieve microservice configuration remote repository by key 'pip-services-feedbacks':
```javascript
{
    "config": {
        "type": "remote",
        ...
        "key": "pip-services-feedbacks" // Microservice config key
    } 
    ...
}
```
...
- path: string - file path where Feedback objects are stored. The object are written into the file in JSON format.
- data: [Feedback] - (optional) array of Feedback objects which is used to initialize the dataset. This option is only used in testing to set a known state on test start.
...
Feedbacks microservice depends on **Storage** microservice. 
